//: Playground - noun: a place where people can play

import UIKit

// Numeros
// UInt8: entero sin signo de 8 bits
// Int32: entero de 32 bits

let minValue = UInt8.min // El valor minimo de un UInt8 es 0
let maxValue = UInt8.max // El valor maximo de un UInt8 es 255

var miEntero: Int = 6

print("Valores de 32 bits: \(Int32.min), \(Int32.max)")
print("Valores de 64 bits: \(Int64.min), \(Int64.max)")


// Numeros Decimales
let pi = 3.14159265 // Es un numero double
let minT = -273.15 // Es un numero double

let meaningOfLife = 42 // Es un numero entero, que vale 42
let anotherPI = 3 + 0.14159265 // Es un double
